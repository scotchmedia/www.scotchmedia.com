...
This section will cover creating a user from a form submission. When committing user submitted data to the database it is import to verify that the content is valid, and return an appropriate error message to the user if it is not. To perform the validation we will you be using [node-validator][node-validator].
